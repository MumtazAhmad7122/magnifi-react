...
function render_isl_cards($slug) {
    ...
    if($q->have_posts()): while($q->have_posts()): $q->the_post();
        ...
        $is_whatsapp = (strpos($f_url, 'whatsapp.com') !== false);
    ?>
    <div class="media-post-group-item" data-search-title="<?php echo esc_attr(strtolower(get_the_title())); ?>" data-search-date="<?php echo esc_attr($date); ?>">
        <div class="bordered-card-container">
            <div class="custom-card-image" style="position: relative; overflow: hidden; height: 220px; background: #000;">
                <?php 
                if (count($video_urls) > 1): 
                    // Case 1: Multiple Videos (Gallery Button)
                ?>
                ...
                <?php 
                elseif (count($video_urls) == 1): 
                    // Case 2: Single Video (Solid iframe Logic)
                    $yt_id = '';
                ...
                <div class="custom-card-footer">
                    ...
                    <div>
                        <?php if(!empty($gal) && is_array($gal)): ?>
                            <button class="btn-download open-isl-gallery" data-gallery='<?php 
                        ...
                        <?php if($f_url && count($video_urls) <= 1): ?>
                            <a href="<?php echo $f_url; ?>" class="btn-download" target="_blank">
                                <?php if($is_whatsapp): ?>
                                    <img src="https://upload.wikimedia.org/wikipedia/commons/6/6b/WhatsApp.svg" style="width:16px; height:16px; filter: brightness(0) invert(1); margin-right:5px;">
                                <?php endif; ?>
                                <?php echo !empty($vid_field) ? 'View Channel' : 'Download'; ?>
                            </a>
                        <?php endif; ?>
                    </div>
                </div>
