      ...
      <Container>
        ...
        <Slider {...settings}>
          {newsletters.map((post) => {
            // 1. Get the featured image URL
            const imgUrl = post._embedded?.['wp:featuredmedia']?.[0]?.source_url;
            
            // 2. PDF Link Logic
            // If ACF returns a string (URL), we use it. 
            // If it returns an object (File Array), we take the .url property.
            const rawPdf = post.acf?.pdf_upload;
            const pdfLink = typeof rawPdf === 'string' ? rawPdf : rawPdf?.url || "#";
                ...
                <div className='main-slide' style={{ border: '1px solid #ddd', borderRadius: '10px', padding: '15px' }}>
                  ...
                  <div className='slt-info text-center mt-3'>
                    ...
                    <Button 
                      ...
                      disabled={pdfLink === "#"}
                    >
                      {pdfLink === "#" ? "No File Available" : "Download PDF"}
                    </Button>
                  </div>
                </div>
              </div>
