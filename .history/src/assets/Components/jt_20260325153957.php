...
function render_isl_cards($slug) {
    ...
    if($q->have_posts()): while($q->have_posts()): $q->the_post();
        ...
        if ($f_url) {
            ...
            // 3. Check for Redirection/Web Page
    // 3. Check for Redirection/Web Page
    elseif (filter_var($f_url, FILTER_VALIDATE_URL) && !empty($f_url)) {
        
        // Use a safer check for file extensions
        $is_file = false;
        ...
        if (!$is_file) {
            // If the URL contains a #, it's a tab link
