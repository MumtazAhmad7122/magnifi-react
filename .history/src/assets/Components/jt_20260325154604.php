    ...
    if($q->have_posts()): while($q->have_posts()): $q->the_post();
        ...
        if (!empty($f_url)) {
            if (strpos($f_url, 'whatsapp.com') !== false) {
                ...
                $btn_icon = '<i class="fa fa-play" style="margin-right:5px;"></i> ';
            } else {
                // Check if it's a file or a web link
                $extensions = array('pdf', 'jpg', 'jpeg', 'png', 'gif', 'zip', 'docx', 'xlsx');
                $path_info = pathinfo(parse_url($f_url, PHP_URL_PATH), PATHINFO_EXTENSION);
                
                if (in_array(strtolower($path_info), $extensions)) {
                    $btn_text = 'Download';
                } else {
                    // It's a redirect or a section link
                    if (strpos($f_url, '#') !== false) {
                        $btn_text = 'View Section';
                        $btn_icon = '<i class="fa fa-arrow-right" style="margin-right:5px;"></i> ';
                    } else {
                        $btn_text = 'Redirect';
                        $btn_icon = '<i class="fa fa-external-link" style="margin-right:5px;"></i> ';
                    }
                }
            }
        }
            ...
            <div class="custom-card-body">
                ...
                <div class="custom-card-footer">
                    ...
                    <div>
                        ...
                        <?php if(!empty($f_url) && count($video_urls) <= 1): ?>
                            <a href="<?php echo esc_url($f_url); ?>" class="btn-download" target="_blank">
...
// --- Deep Linking / Hash Logic ---
    function handleHash() {
        ...
        if (hash) {
            // Find the tab trigger that has a data-target matching the ID in the URL
            var $targetTab = $('.tab-trigger[data-target="' + hash.substring(1) + '"]');
            
            if ($targetTab.length) {
                switchParentTab($targetTab);
                
                // Optional: Smooth scroll to the tab section
                $('html, body').animate({
                    scrollTop: $("#media-main-content").offset().top - 100
                }, 500);
            }
        }
    }
    ...
    // Listen for hash changes (in case the user clicks a redirect link to the same page)
